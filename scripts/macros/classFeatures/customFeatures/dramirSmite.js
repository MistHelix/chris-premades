...
export async function dramirSmite({speaker, actor, token, character, item, args, scope, workflow}) {
    ...
    let selectedOption = await chris.dialog('Rune Infused Blade', menuOptions, 'Use ' + 'Rune Infused Blade' + '?');
    if (!selectedOption) {
        return;
    }
    let update = {};
    ...
    let bonusDamageFormula = damageDiceNum + 'd6[necrotic]';
